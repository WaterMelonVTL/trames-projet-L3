...
export function useUpdateCours() {
  ...
  return useMutation({
    mutationFn: async ({ courseId, newData }) => {
      console.log("Sending update request to server for course:", courseId, "with data:", newData);
      try {
        const response = await api.put(`/cours/${courseId}`, newData);
        console.log("Server response for update request:", response);
        return response; // Return the response body
      } catch (error) {
        console.error("Error during API call for course update:", error);
        throw error; // Ensure the error propagates to trigger onError
      }
    },
    onMutate: async ({ courseId, newData }) => {
      console.log("Optimistically updating cache for course:", courseId, "with data:", newData);

      // Cancel any outgoing refetches
      await queryClient.cancelQueries({ queryKey: ['classes'] });

      // Find the course in all cached weeks
      const classQueries = queryClient.getQueryCache().findAll({
        predicate: query => query.queryKey[0] === 'classes' && query.queryKey[1] === 'week',
      });
      ...
      if (!affectedQueryKey) {
        console.error("Course to update not found in cache:", courseId);
        return;
      }

      // Optimistically update the course in the cache
      const updatedCourses = previousCourses.map(course =>
        String(course.Id) === String(courseId) ? { ...course, ...newData } : course
      );

      queryClient.setQueryData(affectedQueryKey, updatedCourses);
      console.log("Cache updated optimistically for course:", courseId);

      return { previousCourses, affectedQueryKey };
    },
    onSuccess: (data, { courseId, newData }, context) => {
      console.log("Mutation successful for course:", courseId, "with server response:", data);

      if (context) {
        const { affectedQueryKey } = context;

        // Ensure the cache is updated with the server response
        queryClient.setQueryData(affectedQueryKey, oldCourses => {
          const updatedCourses = oldCourses.map(course =>
            String(course.Id) === String(courseId) ? { ...course, ...newData } : course
          );
          console.log("Cache updated with server response for course:", courseId);
          return updatedCourses;
        });
      }
    },
    onError: (error, variables, context) => {
      console.error('Mutation failed for course:', variables.courseId, 'Error:', error);

      if (context) {
        const { previousCourses, affectedQueryKey } = context;
        queryClient.setQueryData(affectedQueryKey, previousCourses);
        console.log("Cache rolled back to previous state for course:", variables.courseId);
      }
    },
    onSettled: () => {
      console.log("Mutation settled. Invalidating queries to ensure consistency.");
      queryClient.invalidateQueries({ queryKey: ['classes'] });
    },
  });
}

export function updateCourseInCache(queryClient, courseId, newData) {
  console.log("Updating course in cache for course:", courseId, "with data:", newData);
  ...
  for (const query of classQueries) {
    ...
    if (courseToUpdate) {
      ...
      queryClient.setQueryData(query.queryKey, updatedCourses);
      console.log("Cache updated for course:", courseId);
      return;
    }
  }
