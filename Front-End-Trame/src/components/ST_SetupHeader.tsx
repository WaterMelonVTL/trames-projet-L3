...
const ST_SetupHeader: React.FC<SetupHeaderProps> = ({ setupStage, totalLayers }) => {
  ...
  return (
    <div className="flex items-center justify-around mb-8">
      ...
      <h1 className={`font-bold text-2xl ml-2 mr-8 ${currentStep === 3 ? 'text-black' : 'text-gray-400'}`}>
        Groupes
      </h1>
      <div className={`rounded-full w-16 h-16 flex items-center justify-center text-3xl font-bold ${currentStep === 4 ? 'bg-blue-500 text-white' : 'bg-gray-500 text-white'}`}>
        4
      </div>
