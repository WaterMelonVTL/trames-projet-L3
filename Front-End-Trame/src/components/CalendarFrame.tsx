...
function CalendarFrame() {
    ...
    const cours = [{'id':123, 'nom': 'HAI101I', 'type': 'CM', 'enseignant': 'M. Dupont', 'salle': 'A5.01', 'jour':1, 'start':1 , 'durée':1, 'offset':0, 'couleur':'#FFB3BA', 'groupe':["2024-L1-A","2024-L1-B","2024-L1-C"], date:"2024-11-11"}, 
            ...
            <div className="border-2 border-black flex flex-row w-full justify-around">
                ...
                {daysOfWeek.map((day, index) => (
                    ...
                    <div key={day} className="flex flex-col flex-grow">
                        ...
                        {rows.map((_, colIndex) => (
                            <>
                                <div key={colIndex} className="bg-white h-28 border border-black hover:bg-gray-300 cursor-pointer relative" onClick={()=> {console.log(`vous avez clické sur ${day} ${crenaux[colIndex].start}`)}} onContextMenu={(e) => {e.preventDefault(); console.log(`right click sur ${day} ${crenaux[colIndex].start}`)}} >
                                    {cours.map((cours) => {
